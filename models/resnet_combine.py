...
class ResNet(nn.Module):
    ...
    def forward(self, x, no_bayes=True, logit_out=False):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        ...
        if no_bayes == True:
            ...
            _, pred_labels = torch.max(pred_labels.data, 1)
            bayes_post = torch.nn.functional.one_hot(pred_labels, 10).float()
            noisy_post = torch.bmm(bayes_post.unsqueeze(1), T).squeeze(1) # softmax output
